...
export default function ChangeOfIllumination({ realEstateId, bulbId } : ChangeOfIlluminationProps) {
    ...
    const oldCalculation: GeneralCalculation = useMemo(() => surveyAnswers && bulbs? calculateOverallFootprint(surveyAnswers, bulbs) : {costs: 0, overallFootprint: 0}, [surveyAnswers, bulbs]);
    const newIllumination: IlluminationCalculation = useMemo(() => surveyAnswers && bulbs? changeBulbs(surveyAnswers, bulbs, bulbId) : {typeOfBulb: bulbId, amountOfIlluminants: 0, costs: 0, overallFootprint: 0}, [surveyAnswers, bulbs, bulbId]);
...
>
  ...
  <ReducedFootprintAndCosts oldCalculation={oldCalculation} newIllumination={newIllumination} /> </GridItem>
  <GridItem colSpan={2} >
      <CarbonFootprintComponent heading={"Reduced footprint"} carbonFootprint={newIllumination.overallFootprint}/> </GridItem>
  <GridItem colSpan={2} > 
