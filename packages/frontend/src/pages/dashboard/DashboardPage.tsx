...
import { ActionPanelFormEngineContext } from './ActionPanelFormEngineContext';

const actionPageSchema: FormSchema = {
  pages: [
    {
      elements: [
...
export const actionPanelSchema: FormSchema = {
  pages: [
    {
      elements: [
        {
          id: 'changeToLED',
          required: false,
          label: 'Change to LED lamps',
          type: 'single-choice',
          options: [
            { value: '00000000-0000-0000-0000-000000000003', display: 'LED 800 lum' },
            { value: '00000000-0000-0000-0000-000000000002', display: 'LED 1300 lum' },
          ],
        },
      ],
    },
      ...
      elements: [
        {
          id: 'switchToHeatPump',
          required: false,
          label: 'Switch to heat pump (with green electricity)',
          type: 'check',
        },
      ],
    },
...
export default function DashboardPage() {
  ...
  const [popUpSchema, setPopUpSchema] = useState<PopUpSchema>(null!);
  ...
  const { value, page, ruleEvaluationResults, validationErrors, verifySubmit, handleValueChanged, setValue } =
    useFormEngine(actionPageSchema);

  const actionPanelFormEngine = useFormEngine(actionPanelSchema);
    ...
    <PopUpContext.Provider
      value={{
        onOpen: (schema: PopUpSchema) => {
          setPopUpSchema(schema);
          popUpActions.onOpen();
        },
      }}>
      <ActionPlanContext.Provider value={{ actionValue: actionValue, setActionValue: setActionValue }}>
        <ActionPanelFormEngineContext.Provider value={{ actionPanelFormEngine: actionPanelFormEngine }}>
          <Flex minH="100%">
            <Flex
              as="aside"
              direction="column"
            ...
            {popUpSchema ? (
              <PopUp
                ...
                onClose={(value) => {
                  setActionValue({ ...actionValue, [popUpSchema]: value });
                  popUpActions.onClose();
                }}
                schema={popUpSchema}
                initialValue={actionValue[popUpSchema]}
              />
            ) : null}
          </Flex>
          <Modal
        ...
        </ActionPanelFormEngineContext.Provider>
