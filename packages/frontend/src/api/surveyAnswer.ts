...
function formatIlluminationSurveyAnswer(
  ...
  if (answer.value.switchOnMode == 'always') {
    const newObject = Object.assign({}, answer);
    newObject.value = { ...answer.value, avgRuntimePerDay: 1 };
    return newObject;
  } else {
    return answer;
  }
...
function applyActions(answers: Array<SurveyAnswer>, actions: FilledActionAnswers): Array<SurveyAnswer> {
  return answers.map((answer) => {
    if (isSurveyAnswerType('illumination', answer)) {
      if (actions.changeBulbs || actions.reduceRuntime) {
...
function calculateIlluminationFootprint(
  ...
  if (usedBulb !== null) {
    ...
    const illuminationPerYear = formattedAnswer.value.avgRuntimePerDay! * 24 * formattedAnswer.value.avgRuntimePerYear!;
    const footprint =
...
export function changeIllumination(
  ...
  if (actions.reduceRuntime && actions.reduceRuntime.values.value.dailyRuntimeReductionInDays != undefined) {
    const newRuntime = actions.reduceRuntime!.values.value.dailyRuntimeReductionInDays;
...
function changeRuntime(
  ...
  newObject.value = {
    ...answer.value,
    avgRuntimePerDay: answer.value.avgRuntimePerDay! - newRuntime,
